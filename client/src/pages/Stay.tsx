  ...
  return (
    <div className="pb-4">
      ...
      <section className="container pt-10 md:pt-20">
        ...
        <div className="mt-8 border-b border-line pb-6 sm:mt-10 sm:pb-8">
          <p className="sv-eyebrow mb-4">{copy.chooseTitle}</p>
          <div className="sv-scrollbar-none -mx-4 flex snap-x gap-2 overflow-x-auto px-4 pb-1 sm:mx-0 sm:flex-wrap sm:px-0" role="group" aria-label={copy.chooseTitle}>
            {FILTERS.map(value => (
              <button
                key={value}
                type="button"
      ...
      {filter === "whole" ? (
        <section className="container mt-9 md:mt-16">
          <div className="grid gap-7 bg-deep px-5 py-7 text-white sm:px-9 md:grid-cols-[1fr_auto] md:items-end md:px-12 md:py-12">
            <div>
            ...
            {visibleUnits.map(unit => (
              <article key={unit.id} className="group border-b border-line pb-8">
                <Link href={`/stay/${unit.id}`} className="relative block overflow-hidden bg-pistachio/10" aria-label={`${unit.title} — ${copy.card.askAbout}`}>
                  <Img src={unit.cover} alt={unit.title} sizes="(min-width: 1280px) 600px, (min-width: 768px) 50vw, 100vw" maxWidth={1280} priority={unit.id === "small-a"} className="aspect-[16/10] w-full object-cover transition-transform duration-500 group-hover:scale-[1.025]" />
      ...
      <section className="container mt-12 md:mt-20">
        ...
        <div className="mt-9 grid gap-6 md:mt-12 md:grid-cols-[0.8fr_1.2fr] md:gap-8">
