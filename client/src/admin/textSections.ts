...
export const TEXT_SECTIONS = ["hero", "highlights", "services", "stay", "events", "pool", "restaurant", "menu", "location", "about", "booking", "guides", "gallery", "brand", "meta"] as const;
...
export function leafPresentation(path: string, sample: string): { kind: TranslateKind; multiline: boolean; maxLength: number } {
  ...
  const multiline = sample.length > 90 || /^(body\d*|intro|description|subtitle|limitBody|qrBody|noAlcoholNote|successBody|fallbackBody|errorBody|answer|planBody|notInLanguage)$/.test(last);
