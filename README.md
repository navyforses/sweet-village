...
| `/guides` | `client/src/pages/Guides.tsx` | Index of the owner's guide articles written in the current language | `shared/content.ts` (`guides` section; seeds in `shared/guideCopy.ts`) |
| `/guides/:slug` | `client/src/pages/GuideDetail.tsx` | One guide article: Markdown body (`shared/markdown.ts`, `client/src/components/Markdown.tsx`), FAQ block, stay CTA, related attractions; `Article` + `FAQPage` + `BreadcrumbList` JSON-LD. A language the article is not written in shows the fallback text with `noindex` | `client/src/seo/jsonld.ts` |
| `/booking` | `client/src/pages/Booking.tsx` | Booking request form, FAQ block (`FAQPage` JSON-LD) and WhatsApp fallback | `shared/booking.ts`, `server/booking.ts`, `api/booking.ts` |
...
| `/admin`, `/admin/login`, `/admin/units(/:unitId)`, `/admin/home`, `/admin/contact`, `/admin/pool`, `/admin/events(/:eventId)`, `/admin/attractions`, `/admin/about`, `/admin/menu(/:categoryId)`, `/admin/guides(/:slug)`, `/admin/texts(/:section)`, `/admin/history(/:section)`, `/admin/bookings` | `client/src/admin/AdminApp.tsx` | **Owner admin panel** (Georgian only, `noindex`, rendered without the public chrome). Edits prices, unit/event copy, galleries, page photos, pool facts, attractions, contact, location, the menu, the guide articles (Markdown subset, FAQ, cover, hidden flag; new guides start hidden, slugs are fixed at creation) and every page text; shows revision history with restore and the booking enquiries. | `shared/content.ts`, `shared/contentSchema.ts`, `api/admin/[action].ts`, `api/content.ts` |
...
| **Owner-editable content (runtime)** | `shared/content.ts` (types, `DEFAULT_CONTENT`), `shared/contentSchema.ts` (zod), `shared/unitCopy.ts`, `shared/venuePhotos.ts` | The compiled constants above are **defaults**. Once the owner saves a section in `/admin`, the saved value (Neon `site_content` table) overrides the default on the public site. Prices, unit and event copy, galleries and captions, homepage/pool/about/events photos, pool facts, attractions, contact, coordinates, the restaurant menu (categories and dishes, including a "temporarily unavailable" flag), the guide articles (`guides`: slug, dates, cover, title/excerpt/body per language, FAQ, related attraction ids, hidden flag; a guide is published in a language only when its title and body exist in it) and every dictionary section (`texts` patches, including the pool and booking FAQ lists) are owner-editable. Defaults for the moved copy live in `shared/unitCopy.ts`, `shared/eventCopy.ts`, `shared/attractionCopy.ts`. |
...
`pnpm build:vercel` runs three steps: `vite build` (client bundle), `vite build --ssr src/entry-server.tsx` (a Node bundle of the same app) and `tsx scripts/prerender.ts`. The prerender step renders every public route in all six languages with React 19's `prerender` (which waits for the lazy pages), writes them as directory index files under `dist/public` (`stay/index.html`, `en/stay/index.html`, …), plus `404.html` and `sitemap.xml` with hreflang alternates. Helpers live in `client/src/seo/prerender.ts`; head tags come from `client/src/seo/Seo.tsx` and are hoisted by React. `entry-server.tsx` lifts React's `progressiveChunkSize` so completed Suspense boundaries are written inline (by default Fizz outlines any boundary over 12.8 KB behind a spinner and a client-side swap script) and fails the build if a page still contains one. Guide articles are only prerendered (and listed in the sitemap, with their own `lastmod` and only their real hreflang alternates) in the languages they are written in.

When `NEON_DATABASE_URL` is available at build time the owner's saved sections are baked into the HTML and embedded as `<script id="sv-content" type="application/json">` so hydration matches; the browser still fetches `/api/content` afterwards. Guide bodies and FAQs are stripped from that embedded JSON on every page except the article's own, which embeds the whole resolved guides section (`contentForEmbedding`). The browser bundle itself carries the seed articles without bodies (`shared/guideCopy.stub.ts`, aliased in `vite.config.ts` for the client build only) and `GuideDetail` imports `shared/guideBodies.ts` on demand, so the main chunk stays free of the article text (212 KB gz main chunk, 15 KB gz bodies chunk).

**Images.** Content photos render through `client/src/components/Img.tsx`, which builds a `srcset` of `/_vercel/image?url=...&w=...&q=75` candidates (Vercel Image Optimization: AVIF/WebP, resized per device; widths in `images.sizes` of `vercel.json`, the Blob host allowed via `remotePatterns`) plus the caller's `sizes` hint; above-the-fold images pass `priority` (eager, `fetchpriority=high`, and React emits a `<link rel="preload">` in the prerendered head). The optimizer only exists on Vercel, so `vite.config.ts` turns it on for Vercel builds and off for local ones (`SWEET_VILLAGE_IMAGE_OPTIMIZATION=0|1` overrides). Every `<Img>` keeps an aspect-ratio class, so nothing shifts while loading: a headless Chromium pass over the static build (mobile viewport) measured CLS 0.000 on `/`, `/en/stay`, `/guides/prometheus-cave` and `/ru/menu`. Lighthouse itself is not available in the build sandbox; run PageSpeed Insights against production after a deploy. Every admin save or restore calls `POST /api/admin/publish`, which fires the Vercel Deploy Hook (`VERCEL_DEPLOY_HOOK_URL`) at most once per 90 seconds; `/api/cron-publish` (03:00 UTC, `CRON_SECRET`) rebuilds if a save slipped through. `/admin` is served from the empty SPA shell (`admin/index.html`) and renders client-side only.
...
The current suite covers locale paths and legacy `?lang=` upgrades, page metadata (canonical, hreflang, Open Graph) and JSON-LD builders, booking validation, booking API behavior (including owner-edited guest limits), menu completeness, venue/inventory data, the content model and its defaults, content resolution, admin authentication, the admin API dispatcher (login, content, upload, translate with mocked Neon and Anthropic clients), maps proxy behavior, authentic copy, asset URL resolution, upload naming, native Node ESM loading, client-side map loading, the guides section (seed articles, availability per language, embedding slimming, schema), the Markdown subset parser, slug transliteration and translation chunking. The most recent local baseline is **192 passing tests and 1 opt-in live Blob credential test skipped** because it requires a real token outside the local sandbox.
